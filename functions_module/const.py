...
consulta_sql2 = """SELECT 
    VDS.ORDER_ID,
    VDS.ORDER_PURCHASE_TIMESTAMP,
    GL.GEOLOCATION_LAT AS latitude,
    GL.GEOLOCATION_LNG AS longitude,
    SUM(IP.PRICE) AS valor,
    SUM(IP.FREIGHT_VALUE) AS frete
FROM CLIENTES CL
INNER JOIN GEOLOCALIZACAO GL ON CL.ZIP_CODE_PREFIX = GL.ZIP_CODE_PREFIX
INNER JOIN VENDAS VDS ON CL.CUSTOMER_ID = VDS.CUSTOMER_ID
INNER JOIN ITENSPEDIDOS IP ON VDS.ORDER_ID = IP.ORDER_ID
WHERE VDS.ORDER_STATUS = 'delivered'
GROUP BY VDS.ORDER_ID, VDS.ORDER_PURCHASE_TIMESTAMP, GL.GEOLOCATION_LAT, GL.GEOLOCATION_LNG;
"""
